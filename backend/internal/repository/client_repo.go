...
func (r *clientRepository) FindAll(filter map[string]interface{}, page, limit int) ([]domain.Client, int64, error) {
	...
	query := r.db.Model(&domain.Client{}).
		Preload("Facilitator"). // Assuming relationship exists in Domain, need to check
		Joins("LEFT JOIN submissions ON submissions.client_id = clients.id").
		Joins("LEFT JOIN users AS facilitators ON clients.facilitator_id = facilitators.id")
	...
	// Count Total (Simple count on filtered clients)
