...
import { Loader2, Plus, Trash2, DollarSign, Tag, ToggleLeft, ToggleRight } from 'lucide-react';
...
const COMPONENT_CATEGORIES = ['REGISTRASI', 'PENETAPAN', 'PENDAMPINGAN', 'BPJPH', 'MUI', 'OPSIONAL'] as const;
...
export default function BillingConfigAdmin() {
    ...
    const handleCreate = async () => {
        ...
        try {
            if (activeTab === 'products') {
                await api.post('/billing-config/product-categories', { 
                    name: newItemName, 
                    description: newItemDesc,
                    business_type_id: parseInt(newItemBusinessTypeId)
                });
            } else if (activeTab === 'scales') {
                await api.post('/billing-config/business-scales', { name: newItemName, description: newItemDesc });
            } else if (activeTab === 'schemes') {
                await api.post('/billing-config/sales-schemes', { name: newItemName, description: newItemDesc });
            } else if (activeTab === 'business_types') {
                await api.post('/billing-config/business-types', { name: newItemName, description: newItemDesc });
                ...
                await api.post('/billing-config/components', {
                    ...
                    business_type_id: newItemBusinessTypeId ? parseInt(newItemBusinessTypeId) : null,
                    product_category_id: newItemProductCategoryId ? parseInt(newItemProductCategoryId) : null,
                    province_id: newItemProvinceId ? parseInt(newItemProvinceId) : null,
                    regency_id: newItemRegencyId ? parseInt(newItemRegencyId) : null,
                    district_id: newItemDistrictId ? parseInt(newItemDistrictId) : null,
                });
            }
    ...
    const handleCreateSchemePrice = async () => {
        ...
        try {
            await api.post('/billing-config/scheme-prices', {
                ...
                <div className="space-y-6 animate-in fade-in">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <button onClick={() => setShowSpForm(!showSpForm)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${showSpForm ? 'bg-gray-100 text-gray-600 hover:bg-gray-200' : 'bg-brand-600 text-white shadow-lg shadow-brand-200 hover:bg-brand-700'}`}>
                        ...
                        {showSpForm && (
                            <div className="p-6 bg-brand-50/30 border-b border-gray-100 space-y-6">
                                ...
                                <div className="flex justify-end gap-3 pt-2">
                                    <button onClick={() => setShowSpForm(false)} className="px-6 py-2.5 text-sm font-bold text-gray-600 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl transition-colors shadow-sm">Batal</button>
                                    <button onClick={handleCreateSchemePrice} disabled={!spForm.sales_scheme_id || !spForm.base_price} className="px-6 py-2.5 bg-brand-600 text-white text-sm font-bold rounded-xl shadow-md shadow-brand-200 hover:bg-brand-700 disabled:opacity-50 disabled:shadow-none transition-all">Simpan Harga</button>
                                </div>
                            </div>
                        )}
                                        ...
                                        {schemePrices.map(sp => (
                                            <tr key={sp.id} className="hover:bg-brand-50/30 group transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <button onClick={() => handleDeleteSchemePrice(sp.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl opacity-0 group-hover:opacity-100 transition-all">
                ...
                <div className="space-y-6 animate-in fade-in">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">Tambah Komponen Biaya Baru</h3>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                            <label className="flex items-center gap-3 cursor-pointer group bg-gray-50 hover:bg-gray-100 px-4 py-2.5 rounded-xl transition-colors">
                            ...
                            <button onClick={handleCreate} disabled={!newItemName} className="px-6 py-2.5 bg-brand-600 hover:bg-brand-700 text-white text-sm font-bold rounded-xl disabled:opacity-50 flex items-center gap-2 shadow-md shadow-brand-200 transition-all">
                                <Plus className="w-4 h-4" /> Simpan Komponen
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {components.length === 0 ? (
                                    <tr><td colSpan={5} className="py-12 text-center text-gray-400">Belum ada komponen biaya.</td></tr>
                                ) : components.map(c => (
                ...
                <div className="space-y-6 animate-in fade-in">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">
                                    Tambah {activeTab === 'schemes' ? 'Skema Penjualan' : activeTab === 'business_types' ? 'Jenis Bidang' : activeTab === 'products' ? 'Jenis Produk' : 'Skala Usaha'} Baru
                                </h3>
                                <p className="text-xs text-gray-500 mt-0.5">Lengkapi data di bawah ini untuk menambahkan master data baru.</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 items-start">
                            ...
                            <button onClick={handleCreate} disabled={!newItemName} className="px-6 py-3 bg-brand-600 text-white text-sm font-bold hover:bg-brand-700 rounded-xl shadow-md shadow-brand-200 flex items-center gap-2 disabled:opacity-50 disabled:shadow-none transition-all whitespace-nowrap w-full md:w-auto justify-center">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {activeTab === 'schemes' && schemes.length === 0 && <tr><td colSpan={2} className="py-8 text-center text-gray-400">Belum ada data</td></tr>}
                                {activeTab === 'schemes' && schemes.map(s => (
                                    <tr key={s.id} className="hover:bg-gray-50/50"><td className="px-6 py-4 font-bold text-gray-800">{s.name}</td><td className="px-6 py-4 text-gray-500">{s.description || <span className="text-gray-300 italic">Kosong</span>}</td></tr>
                                ))}
                                
                                {activeTab === 'business_types' && businessTypes.length === 0 && <tr><td colSpan={2} className="py-8 text-center text-gray-400">Belum ada data</td></tr>}
                                {activeTab === 'business_types' && businessTypes.map(bt => (
                                    <tr key={bt.id} className="hover:bg-gray-50/50"><td className="px-6 py-4 font-bold text-gray-800">{bt.name}</td><td className="px-6 py-4 text-gray-500">{bt.description || <span className="text-gray-300 italic">Kosong</span>}</td></tr>
                                ))}
                                
                                {activeTab === 'products' && products.length === 0 && <tr><td colSpan={3} className="py-8 text-center text-gray-400">Belum ada data</td></tr>}
                                ...
                                {activeTab === 'scales' && scales.length === 0 && <tr><td colSpan={2} className="py-8 text-center text-gray-400">Belum ada data</td></tr>}
